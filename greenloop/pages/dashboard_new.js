...
import { authService, emissionsService, sdgService } from '../utils/api'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const emissions = await emissionsService.getUserEmissions(user.id)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-1 text-gray-600">Welcome, {userData?.name}</p>
        </div>
      </header>
